...
namespace Bango.Base.Csv
{
    ...
    public class CsvFileReader : CsvFileCommon, IDisposable
    {
        ...
        public Dictionary<int, string> ReadFile(List<string> columns)
        {
            string line;
            int i = 0;
    ...
    public class CsvFileWriter : CsvFileCommon, IDisposable
    {
        ...
        public void WriteJsRow(List<string> columns)
        {
            // Verify required argument
            if (columns == null)
                throw new ArgumentNullException("columns");
        ...
        public void writeStartingData()
        {
            //Writer.Write("Ext.define([],function(){ var fullData=[");

            Writer.Write("Ext.define('ERP.DateMap.DateMap',{date_map:function(){var fullData=[");
            Writer.WriteLine();
        }
        public void writeEndingData()
        {
            Writer.Write("]; return fullData;}});");
            Writer.WriteLine();
        }
        ...
        public void WriteDataTabletoJs(DataTable dt)
        {
            if (dt != null & dt.Rows.Count > 0)
            {
                foreach (DataRow dr in dt.Rows)
